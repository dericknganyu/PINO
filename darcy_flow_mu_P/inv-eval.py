...
model.load_state_dict(torch.load("/home/derick/Documents/PINO/darcy_flow_mu_P/files/inv/last_model_res=513_0.234448-relErr-darcyPWC-pino-0.0-noise.pt"))#files/inv/last_model"+ModelInfos+".pt"))
model.eval()
...
#     i += 1
#     x, y = x.cuda(), y.cuda()
#     x = x_normalizer.encode(x).reshape(batch_size, res, res, 1)
...
#     all_test_l2 += myLoss(out.view(batch_size,-1), y.view(batch_size,-1)).item()

#     out = out.reshape(batch_size, res, res)
#     y   = y.reshape(batch_size, res, res)

#     for j in range(batch_size):
#         acc.append(100*(out[j].flatten() == y[j].flatten()).float().sum().item()/len(out[j].flatten()))    

#     # accuracy += ((out.flatten() == y.flatten()).float().sum().item())/len(out.flatten())
...
# plt.title("Input")
# plt.imshow(out[0].detach().cpu().numpy(), cmap=colourMap, extent=[params["xmin"], params["xmax"], params["ymin"], params["ymax"]], origin='lower', aspect = 'auto')#, vmin=0, vmax=1, )
...
# plt.ylabel('y')#, fontsize=16, labelpad=15)
# plt.title("FDM")
# plt.imshow(y[0].detach().cpu().numpy(), cmap=colourMap, extent=[params["xmin"], params["xmax"], params["ymin"], params["ymax"]], origin='lower', aspect = 'auto')#, vmin=0, vmax=1, )
# plt.colorbar()#format=OOMFormatter(-5))
...
# plt.ylabel('y')#, fontsize=16, labelpad=15)
# plt.title("Error")
# plt.imshow(torch.abs(y[0]-out[0]).detach().cpu().numpy(), cmap=colourMap, extent=[params["xmin"], params["xmax"], params["ymin"], params["ymax"]], origin='lower', aspect = 'auto')#, vmin=0, vmax=1, )
# plt.colorbar()#format=OOMFormatter(-5))

# plt.savefig('figures/eval/check-helped.png',dpi=500)
...
fig.tight_layout()
plt.savefig('figures/eval/PINO-DarcyPWC-Inverse-UP.png',dpi=500)
