...
x_cord = torch.linspace(params["xmin"],params["xmax"],res)
y_cord = torch.linspace(params["ymin"],params["ymax"],res)

x_cord, y_cord = torch.meshgrid(x_cord, y_cord)
mollifier = torch.sin(np.pi * x_cord) * torch.sin(np.pi * y_cord) #* 0.001 
...
if os.path.isfile('files/mollifier/lossData_'+TIMESTAMP+'.txt'):
    os.remove('files/mollifier/lossData_'+TIMESTAMP+'.txt')
...
for ep in range(epochs):
    ...
    for x, y in train_loader:
        ...
        out = y_normalizer.decode(out) * mollifier
        y = y_normalizer.decode(y)
        data_loss_train = myloss(out.view(batch_size,-1), y.view(batch_size,-1))
        pde_loss_train  = poisson_loss(out, x.reshape(batch_size, res, res))
        loss = 5*data_loss_train  + pde_loss_train 
        #print(x.shape)
        #print(x[..., 0].shape)
        loss.backward()
    ...
    with torch.no_grad():
        ...
        for x, y in test_loader:
            ...
            pde_loss_test  = poisson_loss(out, x.reshape(batch_size, res, res)).item()

            test_l2 += 5*data_loss_test + pde_loss_test#*0.1
    ...
    t2 = default_timer()
    print("epoch: %s, completed in %.4f seconds. Training Loss: %.4f and Test Loss: %.4f || Train Err:  %.4f | Test Err:  %.4f || TrainPDE Err:  %.4f | TestPDE Err:  %.4f "\
        %(ep+1, t2-t1, train_l2, test_l2, error_train, error_test, error_pde_train, error_pde_test))

    file = open('files/mollifier/lossData_'+TIMESTAMP+'.txt',"a")
    file.write(str(ep+1)+" "+str(train_l2)+" "+str(test_l2)+" "+\
                        str(error_train)+" "+str(error_test)+" "+\
                        str(error_pde_train)+" "+str(error_pde_test)+"\n")

ModelInfos = "_%03d"%(res)+"~res_"+str(np.round(error_test,6))+"~RelL2TestError_"+str(ntrain)+"~ntrain_"+str(ntest)+"~ntest_"+str(batch_size)+"~BatchSize_"+str(learning_rate)+\
            "~LR_"+str(wd)+"~Reg_"+str(gamma)+"~gamma_"+str(step_size)+"~Step_"+str(epochs)+"~epochs_"+time.strftime("%Y%m%d-%H%M%S")  
                            
torch.save(model.state_dict(), "files/mollifier/last_model"+ModelInfos+".pt")
os.rename('files/mollifier/lossData_'+TIMESTAMP+'.txt', 'files/mollifier/lossData'+ModelInfos+'.txt')

dataTrain = np.loadtxt('files/mollifier/lossData'+ModelInfos+'.txt')
steps     = dataTrain[:,0]            
...
plt.savefig("figures/mollifier/Error_VS_TrainingStep"+ModelInfos+".png", dpi=500)
...
model.load_state_dict(torch.load("files/mollifier/last_model"+ModelInfos+".pt"))
model.eval()
...
U_PINO = (y_normalizer.decode(model(ff).reshape(1, res, res)) * mollifier).detach().cpu().numpy()
U_PINO = U_PINO[0] 
...
plt.savefig('figures/mollifier/compare'+ModelInfos+'.png',dpi=500)
