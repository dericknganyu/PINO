...
for ep in range(epochs):
    ...
    for x, y in train_loader:
        ...
        loss = data_loss_train  + 0.2*pde_loss_train #5*data_loss_train  + pde_loss_train 
        #print(x.shape)
        #print(x[..., 0].shape)
        loss.backward()
    ...
    with torch.no_grad():
        ...
        for x, y in test_loader:
            ...
            pde_loss_test  = darcy_loss(out, x.reshape(batch_size, res, res)).item()

            test_l2 += data_loss_test + 0.2*pde_loss_test#5*data_loss_test + pde_loss_test#*0.1
...
ModelInfos = "_NL_%03d"%(res)+"~res_"+str(np.round(error_test,6))+"~RelL2TestError_"+str(ntrain)+"~ntrain_"+str(ntest)+"~ntest_"+str(batch_size)+"~BatchSize_"+str(learning_rate)+\
