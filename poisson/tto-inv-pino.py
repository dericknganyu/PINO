...
################################################################
PATH = "fUG_Square_TrainData=1_TestData=1_Resolution=513X513_Domain=[0,1]X[0,1].hdf5"
#Read Datasets
...
if res == 513:
    test_l2 = 0.041556#0.007091
    timestamp = '20220630-223646'#'20220629-120450'
    epochs0 = 1000
if res == 257:
    test_l2 = 0.036588
    timestamp = '20220629-221257'
if res == 129:
    test_l2 = 0.037628
    timestamp = '20220629-152401'
if res == 65:
    test_l2 = 0.029854
    timestamp = '20220629-131403'
if res == 33:
    test_l2 = 0.031358
    timestamp = '20220629-124711'
    epochs0 = 1000
if res == 17:
    test_l2 = 0.023904
    timestamp = '20220303-215921'

ModelInfos = "_inv_%03d"%(res)+"~res_"+str(np.round(test_l2,6))+"~RelL2TestError_"+str(ntrain0)+"~ntrain_"+str(ntest0)+"~ntest_"+str(batch_size0)+"~BatchSize_"+str(learning_rate0)+\
            "~LR_"+str(wd0)+"~Reg_"+str(gamma0)+"~gamma_"+str(step_size0)+"~Step_"+str(epochs0)+"~epochs_"+timestamp

model.load_state_dict(torch.load("../../FNO/poisson-problem/zmain-paper-data/files/last_model"+ModelInfos+".pt"))
model_0.load_state_dict(torch.load("../../FNO/poisson-problem/zmain-paper-data/files/last_model"+ModelInfos+".pt"))
with torch.no_grad():
    model_0.eval()
...
if os.path.isfile('files/tto-inv/tto-inv/lossData_'+TIMESTAMP+'.txt'):
    os.remove('files/tto-inv/lossData_'+TIMESTAMP+'.txt')
...
for ep in range(epochs):
    ...
    for x, y in train_loader:
        ...
        tv_loss_i = total_variation_loss(out)


        loss = pde_loss_train + alpha*loss_0 +  tv_loss_i
        ...
        pde_loss += pde_loss_train.item()
        tv_loss += tv_loss_i.item()
    ...
    t2 = default_timer()
    print("epoch: %s, completed in %.4f seconds. Loss: %.4f | Error: %4f | PDE-loss: %.4f | TV-loss: %.4f "%(ep+1, t2-t1, train_l2, error, pde_loss, tv_loss))

    file = open('files/tto-inv/lossData_'+TIMESTAMP+'.txt',"a")
    file.write(str(ep+1)+" "+str(train_l2)+" "+str(error)+" "+str(pde_loss)+" "+str(tv_loss)+"\n")
...
torch.save(model.state_dict(), "files/tto-inv/last_model"+ModelInfos+".pt")
os.rename('files/tto-inv/lossData_'+TIMESTAMP+'.txt', 'files/tto-inv/lossData'+ModelInfos+'.txt')

dataTrain = np.loadtxt('files/tto-inv/lossData'+ModelInfos+'.txt')
steps     = dataTrain[:,0]            
lossTrain = dataTrain[:,1]            
errorTrain = dataTrain[:,2]  
PDE_loss = dataTrain[:,3]    
TV_loss = dataTrain[:,4]  
...
plt.plot(steps, PDE_loss, label = 'PDE loss')
plt.plot(steps, TV_loss, label = 'TV loss')
plt.xlabel('epochs')#, fontsize=16, labelpad=15)
plt.ylabel('Loss')
plt.legend(loc = 'upper right')
plt.title("lr = %s test error = %s"%(learning_rate, str(np.round(train_l2,6))))
plt.savefig("figures/tto-inv/Error_VS_TrainingStep"+ModelInfos+".png", dpi=500)
...
model.load_state_dict(torch.load("files/tto-inv/last_model"+ModelInfos+".pt"))
model.eval()
...
plt.savefig('figures/tto-inv/compare'+ModelInfos+'.png',dpi=500)
