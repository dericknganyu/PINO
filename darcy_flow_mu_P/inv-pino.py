...
for ep in range(epochs):
    ...
    for x, y in train_loader:
        ...
        pde_loss_train  = darcy_loss(x.reshape(batch_size, res, res), out)
        tv_loss = total_variation_loss(out).item()
        loss = 0.002*pde_loss_train + data_loss_train #+ 0.01*tv_loss #5*data_loss_train  + pde_loss_train 
        #print(x.shape)
        #print(x[..., 0].shape)
        loss.backward()
    ...
    with torch.no_grad():
        ...
        for x, y in test_loader:
            ...
            pde_loss_test  = darcy_loss(x.reshape(batch_size, res, res), out).item()
            tv_loss = total_variation_loss(out).item()

            test_l2 += 0.002*pde_loss_test + data_loss_test #+ 0.01*tv_loss #5*data_loss_test + pde_loss_test#*0.1  data_loss_test#
...
ModelInfos = "_inv_%03d"%(res)+"~res_"+str(np.round(error_test,6))+"~RelL2TestError_"+str(ntrain)+"~ntrain_"+str(ntest)+"~ntest_"+str(batch_size)+"~BatchSize_"+str(learning_rate)+\
